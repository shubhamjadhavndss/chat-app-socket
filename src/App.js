...
const App = () => {
  ...
  const initializeSocket = (token) => {
    ...
    newSocket.on('newMessage', (message) => {
      ...
      if (message.sender._id !== selectedUser?._id) {
        setUnreadUsers(prev => new Set([...prev, message.sender._id]));
      }
      ...
      if (notificationsEnabled && Notification.permission === 'granted') {
        ...
        console.log('Notifications not shown. Enabled:', notificationsEnabled, 'Permission:', Notification.permission);
      }
    });
  ...
  const loadUsers = async (token) => {
    try {
      ...
      setUsers(data);
      const unreadUserIds = data.filter(user => user.hasUnread).map(user => user._id);
      setUnreadUsers(new Set(unreadUserIds));
    } catch (error) {
      ...
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
          ...
          <p className="text-center mt-4">
            ...
            <button
              onClick={() => { setIsLogin(!isLogin); setError(''); setAuthError(''); }}
      ...
      <div className="w-1/3 bg-white border-r border-gray-200 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {users.map(user => {
            ...
            const hasUnread = unreadUsers.has(user._id);
            return (
              <div
                key={user._id}
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {hasUnread && (
                    <div className="bg-red-500 text-white text-xs px-2 py-1 rounded-full">New</div>
                  )}
                </div>
              </div>
            );
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map(message => (
                ...
                >
                  <div
                    className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${message.sender._id === user.id ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'}`}
                  >
